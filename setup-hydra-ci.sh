...
for JOB in test1 test2; do
install -o 122 -g 122 -m u=rwx,go=rx -d /var/tmp/hydra-ci/{,"$JOB/"}
install -o 122 -g 122 -m u=rw,go=r /dev/fd/0 "/var/tmp/hydra-ci/$JOB/flake.nix" <<NIX
{
  inputs = {
    entropy = {
